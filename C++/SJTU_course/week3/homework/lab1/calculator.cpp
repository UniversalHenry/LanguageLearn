...
void error(const string &c,const string &s){
	throw c+s;
	return;
}
struct Token {
...
double primary() // Processes semicolons, numbers and returns variables
{
	...
	{	
		...
		t = ts.get();
		if (t.kind == assign) set_value(s,expression()); // If there is an assignment symbol then update the value of variable
		else ts.unget(t);
		return get_value(s); // Return the value of the variable
	}
	default:
...
{	
	...
	//if () error("name expected in assignment"); // If there is no name return an error
	//string name = t.name; // Read the name in 'name'
	if (is_declared(s) && isdigit(value)) {
