...
// Basic infrastructure for binary input and output
struct Bin
{
	...
	bool operator==(const Bin &b) const
	{
		if (num == b.num)
		{
			...
			int r = num % 8;
			for (int i = 0; i < n; i++)
				if (int(con[i]) - int(b.con[i]) != 0)
					return false;
			for (int i = 0; i < r; i++)
			{
				unsigned char c = 1 << (7 - i);
				if (int(con[n] & c) - int(b.con[n] & c) != 0)
					return false;
			}
			return true;
		}
...
{
  public:
	// Output the encoded content according to the input
	vector<unsigned char> encode(const vector<unsigned char> &str);
	// Output the decoded content according to the input
	vector<unsigned char> decode(const vector<unsigned char> &str);
	// Set the top index of Hfm_tree to 0, see Huffman::Hfm_tree_top
	Huffman() : Hfm_tree_top(0){};

  private:
	// Count the frequency of the input char
	map<unsigned char, int> Freq;
	// Generate the most frequent tree by a priority_queue,
	// the first int represents frequency while the second represents index
	priority_queue<pair<int, int>, vector<pair<int, int> >, cmp> Freq_index;
	// Hfm_tree, represents< father node index, < left son node index, right son node index > >
	map<int, vector<int> > Hfm_tree;
	// Record of the top of Hfm_tree, -1 after every combine of two node
	int Hfm_tree_top;
	// Binary index of every input char
	map<unsigned char, Bin> Hfm_index;

	// Get the right son node index
	int right_son_node(int i);
	// Get the right son node index
	int left_son_node(int i);
	// Combine two nodes to generate a father node
	void combine(pair<int, int> n1, pair<int, int> n2);
	// Generate the Hfm_index according to Hfm_tree
	void gen_hfm_index(int top, Bin b);

	// Generate the code description in binary form
	Bin gencode();
	// Generate the encoded content of input
	Bin encode_content(const vector<unsigned char> &str);
	// Load the encoded binary code, generate the pointer of initial content and the Hfm_index
...
{
	if (Hfm_tree.count(i) == 0)
		return 1000;
	if (Hfm_tree[i].size() <= 1)
		return 500;
	return Hfm_tree[i][1];
}
