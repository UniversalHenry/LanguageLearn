...
int main()
{
    while(true){
        ...
        // try here 
            ...
            cout<<"Please input the width:"<<width;
            // cin>>width;
            cout<<"Please input the height:"<<height;
            // cin>>height;

            vector<vector<int> > maze = generate(width, height);
            // printMaze(maze);
            int start_x = 0, start_y = 0, end_x = width - 1, end_y = height - 1;
            cout << "Please input the start point:("<<start_x<<","<<start_y<<")";
            // cin >> start_x >> start_y;
            cout << "Please input the end point:("<<end_x<<","<<end_y<<")";
            // cin >> end_x >> end_y;
            Point startp(start_x, start_y);
            Point endp(end_x, end_y);
            for(int i = 0; i < 3; i++){
                maze[start_y + i][start_x + i] == 1;
                maze[end_y - i][end_x - i] == 1;
            }

            solve(maze, startp, endp);
            // printMaze(maze);
